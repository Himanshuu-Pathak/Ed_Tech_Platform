    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try{
                ...
                if(data ?.success){ // optional chaining operator (?.)
                ...
                >
                    {reviews.map((review,index) =>(
                        <SwiperSlide key={index}>
                            <div className="bg-slate-950 p-4 text-slate-100 rounded-lg">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col">
                                        <h1 className="font-semibold text-slate-100">{`${review?.user?.firstName} ${review?.user?.lastName}`}</h1>
                                        <h2 className="text-[12px] font-medium text-slate-500">{review?.course?.courseName}</h2>
                                    </div>
                                </div>

                                <p className="font-medium text-slate-100">
                                    {review?.review.split(" ").length > truncateWords
                                      ? `${review?.review.split(" ").slice(0,truncateWords).json(" ")}...`
                                      : `${review?.review}`}
                                </p>
