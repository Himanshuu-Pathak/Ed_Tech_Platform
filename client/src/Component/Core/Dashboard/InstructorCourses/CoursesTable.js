  ...
  return (
    <>
      <Table className="rounded-xl border border-richblack-800">
        <Thead >
          <Tr className="flex gap-x-10 rounded-t-md border-b border-b-richblack-800 px-6 py-2 text-slate-100">
              ...
              >
                <Td colSpan={1} className="flex flex-1 gap-x-4 p-3">
                  ...
                  <div className="flex flex-col gap-1 justify-between">
                    <p className="text-lg font-semibold text-slate-5 mt-3 uppercase truncate tracking-wide">
                      {course.courseName}
                    </p>
                    <ul style={{ listStyle: 'none', padding: 0 }} className="tracking-wider">
                      {(course.courseDescription || "").split('\n').splice(0, 1).map((line, index) => (
                        <li key={index} style={{ display: 'flex', alignItems: 'flex-start' }} className="text-xs text-slate-300">
                    ...
                    {course.status === COURSE_STATUS.DRAFT ? (
                      <p className="flex w-fit flex-row items-center gap-2 rounded-full bg-slate-700 px-2 py-[2px] text-[12px] font-medium text-pink-100 uppercase tracking-wider">
                        <HiClock size={14} />
                        Drafted
                      </p>
                    ) : (
                      <p className="flex w-fit flex-row items-center gap-2 rounded-full bg-slate-700 px-2 py-[2px] text-[12px] font-medium text-yellow-100 uppercase tracking-wider">
                        <div className="flex h-3 w-3 items-center justify-center rounded-full bg-yellow-100 text-slate-700">
                          <FaCheck size={8} />
                        </div>
                        Published
                      </p>
                ...
                <Td className="text-sm font-medium text-slate-100 mb-1 tracking-wider uppercase">
                ...
                <Td className="text-sm font-medium text-slate-100 tracking-wider uppercase">
                  <button
                    disabled={loading}
                    onClick={() => {
                    ...
                    title="Edit"
                    className="pr-2 transition-all duration-200 hover:scale-110 hover:text-green-300 mr- mb-"
                  >
                    <FiEdit2 size={20} />
                  </button>
                  <button
