          ...
          steps.map((item) => (
            <Fragment key={item.id}>
              <div className={`grid place-items-center aspect-square rounded-full w-[34px] border select-none
                ...
                ${item.id > step && "border-slate-700 bg-richblack-800 text-slate-300"}
              `} >
                {
                  item.id < step ? <FaCheck className='font-bold text-slate-900' /> : item.id
                }
              </div>

              {
                item.id !== steps.length && (
                  ...
                  ${item.id < step ? "border-yellow-50" : "border-slate-500"}
                  `} >
                    </div>
                  </>
                )
        ...
        <div className='hidden md:flex justify-between select-none ' >
          {
            steps.map(item => (
              ...
              ${item.id <= step ? "text-slate-50" : "text-slate-500"}`} >
                {item.title}
              </div>
            ))
          }
